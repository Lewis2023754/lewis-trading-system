#!/usr/bin/env python3
"""
SNAP v2 - Zone Reversal Strategy
SL=150pts | RR=1:3 | Max 1 flip per session
Sessions: 02:30 UTC (MORNING) & 14:30 UTC (EVENING)
Backtest: 94.2% WR over 6 months
"""
...
SPOT_BASE  = os.getenv("SPOT_BASE", "https://testnet.binance.vision")
...
def tg(msg):
    try:
        requests.post(
            ...
            json={"chat_id": TG_CHAT, "text": f"SNAP\n{msg}"},
            timeout=10
        )
...
# ── Main loop ────────────────────────────────────────────
log.info("SNAP v2 starting - SL=150 RR=1:3 MaxReentry=1")
tg("SNAP v2 Online\nSL=150 | RR=1:3 | MaxReentry=1\nSessions: 02:30 & 14:30 UTC")
...
while True:
    try:
        ...
        for nm, S in sessions.items():
            ...
            if not S["active"] or S["over"]: continue
            try:
                ask, bid = get_price()
            except Exception as e:
            ...
            if S["trade"]:
                if S["dir"]=="LONG":
                    hit_tp = bid >= S["tp"]; hit_sl = bid <= S["sl"]
                else:
                    hit_tp = ask <= S["tp"]; hit_sl = ask >= S["sl"]
                if hit_tp:
                    sl_p = min(S["zone"]["size"], SL_CAP); tp_p = round(sl_p*RR,1)
                    pnl = round(tp_p*S["qty"],2)
                    S["pnl"]+=pnl; S["trade"]=False; S["over"]=True
                    log_trade(nm,S["dir"],S["entry"],S["sl"],S["tp"],S["qty"],"WIN",pnl,S["entries"])
                    log.info(f"[{nm}] WIN +${pnl}")
                    tg(f"WIN [{nm}]\n{S['dir']}\nEntry: ${S['entry']}\nTP: ${S['tp']}\nPnL: +${pnl}\nSession PnL: ${round(S['pnl'],2)}\nSession done")
                elif hit_sl:
                    sl_p = min(S["zone"]["size"], SL_CAP)
                    pnl = round(-sl_p*S["qty"],2)
                    S["pnl"]+=pnl; S["trade"]=False
                    close_side = "SELL" if S["dir"]=="LONG" else "BUY"
                    place_order(close_side, S["qty"])
                    log_trade(nm,S["dir"],S["entry"],S["sl"],S["tp"],S["qty"],"LOSS",pnl,S["entries"])
                    log.info(f"[{nm}] SL hit ${pnl} entries={S['entries']}")
                    if S["entries"] >= MAX_REENTRY+1:
                        S["over"]=True
                        tg(f"LOSS [{nm}]\n{S['dir']}\nSL hit\nPnL: ${pnl}\nSession PnL: ${round(S['pnl'],2)}\nSession done")
                    else:
                        z=S["zone"]; sl_p2=min(z["size"],SL_CAP); tp_p2=round(sl_p2*RR,1)
                        if S["dir"]=="LONG":
                            S["dir"]="SHORT"; S["entry"]=round(z["high"],2)
                            S["sl"]=round(z["high"]+sl_p2,2); S["tp"]=round(z["high"]-tp_p2,2)
                            place_order("SELL", S["qty"])
                        else:
                            S["dir"]="LONG"; S["entry"]=round(z["low"],2)
                            S["sl"]=round(z["low"]-sl_p2,2); S["tp"]=round(z["low"]+tp_p2,2)
                            place_order("BUY", S["qty"])
                        S["trade"]=True; S["entries"]+=1
                        log_trade(nm,S["dir"],S["entry"],S["sl"],S["tp"],S["qty"],"ENTRY",0,S["entries"])
                        tg(f"LOSS [{nm}]\nSL hit | PnL: ${pnl}\nSession PnL: ${round(S['pnl'],2)}\n---\nFLIP #{S['entries']} {S['dir']}\nEntry: ${S['entry']}\nSL: ${S['sl']}\nTP: ${S['tp']}")
                else:
                    log.info(f"[{nm}] {S['dir']} entry={S['entry']} sl={S['sl']} tp={S['tp']} ask={round(ask,2)} bid={round(bid,2)}")
                time.sleep(5); continue
            if not S["zone"]:
                try:
                    candles = get_klines()
                    ...
                    if z:
                        S["zone"]=z; sl_p=min(z["size"],SL_CAP); tp_p=round(sl_p*RR,1)
                        log.info(f"[{nm}] Zone H={z['high']} L={z['low']} size={z['size']}")
                        tg(f"Zone found [{nm}]\nH: ${z['high']} | L: ${z['low']}\nSize: {z['size']}pts\nSL: {sl_p}pts | TP: {tp_p}pts")
                except Exception as e:
                    log.error(f"Zone error: {e}")
                time.sleep(5); continue
            if S["entries"] > MAX_REENTRY and not S["trade"]:
                S["over"]=True; continue
            z = S["zone"]
            if ask >= z["high"]: S["touched_high"]=True
            if bid <= z["low"]:  S["touched_low"]=True
            if S["touched_high"] and ask < z["high"] and not S["trade"]:
                sl_p=min(z["size"],SL_CAP); tp_p=round(sl_p*RR,1)
                S["dir"]="SHORT"; S["entry"]=round(z["high"],2)
                S["sl"]=round(z["high"]+sl_p,2); S["tp"]=round(z["high"]-tp_p,2)
                S["trade"]=True; S["touched_high"]=False; S["entries"]+=1
                place_order("SELL", S["qty"])
            ...
            if S["touched_low"] and bid > z["low"] and not S["trade"]:
                sl_p=min(z["size"],SL_CAP); tp_p=round(sl_p*RR,1)
                S["dir"]="LONG"; S["entry"]=round(z["low"],2)
                S["sl"]=round(z["low"]-sl_p,2); S["tp"]=round(z["low"]+tp_p,2)
                S["trade"]=True; S["touched_low"]=False; S["entries"]+=1
                place_order("BUY", S["qty"])
                log.info(f"[{nm}] LONG entry={S['entry']} sl={S['sl']} tp={S['tp']} #{S['entries']}")
                tg(f"LONG [{nm}] Entry #{S['entries']}\nEntry: ${S['entry']}\nSL: ${S['sl']}\nTP: ${S['tp']}\nQty: {S['qty']} BTC\nZone: ${z['high']} - ${z['low']}")
                time.sleep(5); continue
            log.info(f"[{nm}] Ask={round(ask,2)} Bid={round(bid,2)} | H={z['high']} L={z['low']} | TH={S['touched_high']} TL={S['touched_low']}")
        time.sleep(5)
    except KeyboardInterrupt:
        log.info("SNAP stopped"); tg("SNAP stopped"); break
    except Exception as e:
        log.error(f"Loop error: {e}"); time.sleep(10)
